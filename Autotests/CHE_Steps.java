...
public class Steps {
    ...
    @Step("Отправляем запрос в ФП События по кредитной истории")
    public ValidatableResponse responseGetEvents(String hostFP, Cookies cookiesE) {
        return RestAssured.given()
                .cookies(cookiesE)
                .relaxedHTTPSValidation()
                .log().all()
                .when().get("http://" + hostFP + ".vm.mos.cloud.sbrf.ru:9080/ch-event-app/v1.0/rest/getEvents")
                .then().log().all().statusCode(200);
    }


    @Step("Отправляем запрос в ФП События по кредитной истории")
    public ValidatableResponse getEvent(String userLogin) {
        ValidatableResponse logIn = login(userLogin);
        Assert.assertEquals(logIn.extract().body().xmlPath().get(RESPONSE_STATUS_CODE), ZERO);
        ...
        String hostFP = hostUfs;
        switch (hostFP) {
            case "nginx-and-ift-b3.testonline.sberbank.ru":
                hostFP = "tklia-efs001617";
            case "nginx-and-ift-b1.testonline.sberbank.ru":
                hostFP = "tklia-efs001616";
                break;
        }

        switch (hostUfs) {
            case "nginx-and-ift-b3.testonline.sberbank.ru":
                hostUfs = "tklia-efs000421.vm.mos.cloud.sbrf.ru:9080";
            case "nginx-and-ift-b1.testonline.sberbank.ru":
                hostUfs = "tklia-efs000421.vm.mos.cloud.sbrf.ru:9080";
                break;
        }
        ...
        Cookies cookiesE = createSessionResponse.extract().response().getDetailedCookies();

        ValidatableResponse getEventsResponse = responseGetEvents(hostFP, cookiesE);
    ...
    public ValidatableResponse responseHealthCheck() {
        return RestAssured.given()
                ...
                .header(X_GW_HOST, "127.0.0.1:8080")
                .header(X_FORWARDED_FOR, "255.255.255.0")
                .when().get("http://10.53.170.67:9080/ch-event-app/v1.0/healthcheck")
                .then().log().all().statusCode(200);
    }
    ...
    public ValidatableResponse responseEnvironmentProduct() {
        return RestAssured.given()
                ...
                .header(X_GW_HOST, "127.0.0.1:8080")
                .header(X_FORWARDED_FOR, "255.255.255.0")
                .when().get("http://10.53.170.67:9080/ch-event-app/v1.0/environment/product")
                .then().log().all().statusCode(200);
    }

    @Step("Отправляем запрос в Единое согласие")
    public ValidatableResponse responseGetCRHEventConsent(String userLogin) {
        return RestAssured.given()
                .cookies(getCookie(userLogin))
                .relaxedHTTPSValidation()
                .log().all()
                .when().post("http://" + hostFP + ".vm.mos.cloud.sbrf.ru:9080/ch-event-app/v1.0/consent?cmd=START&name=CRHEventConsent")
                .then().log().all().statusCode(200);
    }

    @Step("Отправляем запрос в Единое согласие on-enter")
    public ValidatableResponse responseOnEnter(String pid) {
        return RestAssured.given()
                .relaxedHTTPSValidation()
                .log().all()
                .when().post("http://10.106.212.153:9080/control-consent-unify/v1.0/workflow?cmd=EVENT&name=on-enter&pid=" + pid)
                .then().log().all();
    }

    @Step("Отправляем запрос в Единое согласие on-return")
    public ValidatableResponse responseOnReturn(String pid) {
        return RestAssured.given()
                .relaxedHTTPSValidation()
                .log().all()
                .when().post("http://" + hostFP + ".vm.mos.cloud.sbrf.ru:9080/ch-event-app/v1.0/consent?cmd=EVENT&name=on-return&pid=" + pid)
                .then().log().all().statusCode(200);
    }
